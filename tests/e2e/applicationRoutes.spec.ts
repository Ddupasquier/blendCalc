...
for (const route of authenticatedRoutes) {
	test(
		`${route.path} renders its authenticated route shell without browser errors`,
		{ tag: "@compatibility" },
		async ({ page }) => {
...
test(
	"Fridge and Shopping List preserve browser Back and Forward history",
	{ tag: "@compatibility" },
	async ({ page }) => {
		...
		await expect(page).toHaveURL(/\/ingredients\/shopping$/);
		await expect(
			page.getByRole("tab", { name: /Shopping List/ }),
		).toHaveAttribute("aria-selected", "true");
	},
);

test(
	"signed-out visitors are redirected away from protected routes",
	{ tag: "@compatibility" },
