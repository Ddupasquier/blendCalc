...
describe("API catalog readiness audit helpers", () => {
	...
	it("summarizes current readiness without treating every blocker as human review", () => {
		const summary = buildCatalogReadinessSummary([
			...
			{
				api: "withheld",
				reasons: ["missing_current_revision", "missing_field_provenance:brandOwner"],
				issues: [
					{ issueCode: "CATALOG_REVISION_MISSING", owner: "safe_automated_repair" },
					{ issueCode: "CATALOG_FIELD_PROVENANCE_MISSING", owner: "safe_automated_repair" },
				],
			},
			{
				api: "withheld",
				reasons: ["missing_required_field:ingredients"],
				issues: [{ issueCode: "CATALOG_REQUIRED_FIELD_MISSING", owner: "catalog_review" }],
			},
			{
				api: "withheld",
				reasons: ["field_source_not_redistributable"],
				issues: [{ issueCode: "API_REDISTRIBUTION_REVIEW_REQUIRED", owner: "external_review" }],
			},
		]);
	...
	it("treats only missing rollout contracts as temporarily unavailable", () => {
		expect(isCatalogReadinessContractUnavailable({
			code: "42703",
		...
		expect(isCatalogReadinessContractUnavailable(new Error("network failed")))
			.toBe(false);
	});
});
