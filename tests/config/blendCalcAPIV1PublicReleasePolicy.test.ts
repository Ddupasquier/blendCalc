...
import {
	BLENDCALC_API_V1_REQUIRED_PUBLIC_TERMS_REVIEWS,
	BLENDCALC_API_V1_ACCESS_POLICY,
} from "$lib/blendCalcAPI/v1/blendCalcAPIAccessPolicy";
...
describe("blendCalcAPI v1 public-release policy", () => {
	it("keeps public access blocked until every terms area has professional review", () => {
		...
		expect(BLENDCALC_API_V1_REQUIRED_PUBLIC_TERMS_REVIEWS).toEqual([
			"acceptable-use",
			"privacy",
	...
	it.each(routePaths)(
		"routes %s through the shared access boundary",
		(routePath) => {
			const route = readFileSync(routePath, "utf8");
			expect(route).toContain("hasBlendCalcAPIV1CatalogReadAccess");
