...
describe("blendCalcAPI v1 publication readiness migration", () => {
	...
	it("backfills ingredient provenance only from exact stored USDA evidence", () => {
		...
		expect(migration).toContain("lower(btrim(product.food ->> 'ingredients'))");
	...
	it("backfills category provenance only through exact stored category mappings", () => {
		...
		expect(migration).toContain("category_mapping.confidence = 'exact'");
		expect(migration).toContain(
	...
	it("requires canonical provenance and normalized nutrition before publication", () => {
		expect(migration).toContain("blendcalc_api_v1_product_readiness_reasons");
