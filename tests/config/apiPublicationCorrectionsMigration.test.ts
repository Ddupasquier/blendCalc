...
describe("API publication corrections and holds migration", () => {
	it("keeps reports and holds private to the trusted server", () => {
		for (const table of ["api_publication_concerns", "api_publication_holds"]) {
			...
			expect(migration).toMatch(
				new RegExp(`revoke all on table public\\.${table}\\s+from public, anon, authenticated;`),
			);
	...
	it("uses exact subject references and one active hold per subject", () => {
		for (const subject of ["product", "image", "dataset", "source"]) {
			expect(migration).toContain(`api_publication_holds_active_${subject}_unique`);
		}
		expect(migration).toContain("shared_product_id uuid references public.shared_products");
		expect(migration).toContain("food_image_asset_id uuid references public.food_image_assets");
		expect(migration).toContain("dataset_key text references public.generic_food_datasets");
		expect(migration).toContain("source_key text references public.product_data_sources");
	});
	...
	it("withholds held products and source releases without deleting history", () => {
		...
		expect(migration).toContain("not public.blendcalc_api_v1_source_has_active_hold");
