...
describe("API v1 server request boundary", () => {
	beforeEach(() => {
		vi.clearAllMocks();
	...
	it("returns a documented API error when rate-limit storage fails", async () => {
		...
		expect(JSON.stringify(payload)).not.toContain("private rate-limit database detail");
	});
