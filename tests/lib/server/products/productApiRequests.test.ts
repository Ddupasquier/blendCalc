...
describe("fetchCachedProductApiJson", () => {
	...
	it("returns a fresh persistent cache entry without an external request", async () => {
		...
		await expect(fetchCachedProductApiJson({
	...
	it("caches an accepted not-found response", async () => {
		const cacheStore = {
			read: vi.fn().mockResolvedValue(null),
			write: vi.fn().mockResolvedValue(undefined),
		};
		...
		await expect(fetchCachedProductApiJson<null>({
			provider: "test-provider",
			requestKind: "barcode-product",
			cacheValue: "missing",
			url: "https://example.com",
		...
		expect(cacheStore.write).toHaveBeenCalledWith(
			expect.objectContaining({ statusCode: 404, response: null }),
		);
	});
});
