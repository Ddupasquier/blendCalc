...
describe("getFoodPassportPresentation", () => {
	it("summarizes an evolving verified catalog record without exposing technical source ranking", () => {
		...
		expect(passport.availabilityRows).toEqual(
			expect.arrayContaining([
				expect.objectContaining({
					label: "Nutrition",
					value: "2 reported nutrient values",
				}),
				expect.objectContaining({ label: "Ingredients", available: true }),
				expect.objectContaining({
					label: "Package safety",
					value: "Contains details available",
				}),
				expect.objectContaining({
					label: "Field history",
					value: "2 field sources recorded",
				}),
			]),
		);
	...
	it("distinguishes a private food and generic package-label applicability", () => {
		const personal = getFoodPassportPresentation(
			makeFood({
				foodIdentityType: "private-custom",
				trustStatus: "user-private",
			}),
		);
		const generic = getFoodPassportPresentation(
			makeFood({
		...
		expect(
			generic.availabilityRows.find((row) => row.label === "Package safety"),
