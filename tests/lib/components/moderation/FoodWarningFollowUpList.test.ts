...
describe("FoodWarningFollowUpList", () => {
	it("separates product corrections from policy-owned follow-up work", () => {
		...
		expect(screen.getByText(/Ingredients, Allergen declaration/u)).toBeInTheDocument();
		expect(screen.getByRole("link", { name: "Review product and correction status" }))
			.toHaveAttribute(
				"href",
				"/profile/privileged-tools/catalog-review-work/products/product-id",
			);
		expect(screen.getByText("Owner: Food warning policy review"))
			.toBeInTheDocument();
	});
	...
	it("does not add an empty second queue", () => {
		...
		expect(screen.queryByText("Warning follow-up work")).not.toBeInTheDocument();
	});
});
