...
describe("TutorialOverlay", () => {
	it("draws a rounded spotlight around the current feature", async () => {
		...
		expect(
			screen.getByText("The highlighted area contains ingredient search."),
		).toBeInTheDocument();

		viewFrame.remove();
	});

	it("temporarily reveals a semantic disclosure and restores its saved state", async () => {
		const viewFrame = document.createElement("main");
		viewFrame.className = "view-frame";
		const disclosure = document.createElement("details");
		disclosure.dataset.tutorialReveal = "mix-add-ingredients";
		disclosure.dataset.expanded = "false";
		const target = document.createElement("article");
		target.dataset.tutorialTarget = "mix-ingredient-option";
		target.getBoundingClientRect = () =>
			({
				top: 100,
				right: 320,
		...
		expect(target).not.toHaveAttribute("data-tutorial-active");

		viewFrame.remove();
	});

	it("keeps an unavailable target as a centered, usable step", async () => {
		const { container } = render(TutorialOverlay, {
			props: {
				open: true,
				pathname: "/ingredients/fridge",
		...
		await waitFor(() =>
			expect(container.querySelector(".tutorial-shade-fallback")).toBeVisible(),
		);
		expect(
			container.querySelector(".tutorial-spotlight"),
