...
const resolveResponse = (
	...
	const resolved =
		specification.components.responses[response.$ref.slice(prefix.length)];
...
export const expectBlendCalcAPIV1ResponseToMatchOpenAPI = async ({
	method = "get",
	path,
	response,
}: {
	...
	if (!operation)
		throw new Error(
			`OpenAPI operation not found: ${method.toUpperCase()} ${path}`,
		);
	const declaredResponse = operation.responses[String(response.status)];
	if (!declaredResponse) {
		throw new Error(
			`OpenAPI does not document HTTP ${response.status} for ${method.toUpperCase()} ${path}`,
		);
	}
	const schema =
		resolveResponse(declaredResponse).content?.["application/json"]?.schema;
	if (!schema)
		throw new Error(
			`JSON schema not found for ${method.toUpperCase()} ${path} HTTP ${response.status}`,
		);
