...
export const API_PUBLICATION_REPORTER_TYPES = [
	"user",
	"provider",
	"brand",
	"rights-holder",
...
export const API_PUBLICATION_CONCERN_TYPES = [
	"product-correction",
	"rights-or-license",
	"attribution",
	"privacy",
...
export const API_PUBLICATION_SUBJECT_TYPES = [
	"product",
	"image",
	"dataset",
	"source",
...
export type ApiPublicationReporterType =
	(typeof API_PUBLICATION_REPORTER_TYPES)[number];
export type ApiPublicationConcernType =
	(typeof API_PUBLICATION_CONCERN_TYPES)[number];
export type ApiPublicationSubjectType =
	(typeof API_PUBLICATION_SUBJECT_TYPES)[number];

export type CreateApiPublicationConcernInput = {
	reporterType: ApiPublicationReporterType;
	contactName?: string;
	contactEmail: string;
	reporterUserId?: string;
	concernType: ApiPublicationConcernType;
	subjectType: ApiPublicationSubjectType;
...
const normalizeOptionalText = (value: string | undefined, maximumLength: number) => {
...
const normalizeSubjectReference = (
	...
	if (subjectType === "image") return UUID_PATTERN.test(normalized) ? normalized : null;
...
export const createApiPublicationConcern = async (
	input: CreateApiPublicationConcernInput,
) => {
	...
	const fingerprint = createHash("sha256")
		.update(`${input.subjectType}\0${subjectReference}\0${input.concernType}\0${contactEmail}\0${details}`)
		.digest("hex");
	const readDuplicate = () => admin
		.from("api_publication_concerns")
		.select("id, status")
	...
	const { data, error } = await admin
		...
		.insert({
			...
			urgency:
				input.concernType === "privacy" || input.concernType === "rights-or-license"
					? "urgent"
					: "normal",
		})
		.select("id, status")
...
export const readApiPublicationReviewQueue = async () => {
	const admin = getSupabaseAdminClient();
...
export const resolveApiPublicationConcern = async ({
	concernId,
	status,
	resolutionAction,
	resolutionNote,
...
export type ApiPublicationHoldSubject =
...
const readActiveApiPublicationHold = async (
	subjectType: ApiPublicationSubjectType,
...
export const placeApiPublicationHold = async ({
	subject,
	reasonCode,
	publicMessage,
	internalNote,
	...
	subject: ApiPublicationHoldSubject;
	reasonCode:
		| "accuracy-review"
		| "rights-review"
		| "attribution-review"
	...
	if (!normalizedReference || !publicMessage.trim() || !internalNote.trim()) return null;
	const target = await findConcernSubject(subject.subjectType, normalizedReference);
	if (!target) return null;
	const existingHold = await readActiveApiPublicationHold(
		subject.subjectType,
		target,
	);
	...
	const { data, error } = await getSupabaseAdminClient()
		...
		.single();
	if (error?.code === "23505") {
		return readActiveApiPublicationHold(subject.subjectType, target);
	}
	if (error) throw error;
	return data;
};

export const releaseApiPublicationHold = async ({
	holdId,
	actorUserId,
	releaseNote,
}: {
