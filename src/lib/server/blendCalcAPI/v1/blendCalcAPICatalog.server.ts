import {
	BLENDCALC_API_V1,
	type BlendCalcAPIV1Category,
	type BlendCalcAPIV1CompatibilityEvaluation,
	type BlendCalcAPIV1FieldSource,
	type BlendCalcAPIV1Image,
	type BlendCalcAPIV1Pagination,
	type BlendCalcAPIV1Product,
	type BlendCalcAPIV1ProductRevisionChange,
	type BlendCalcAPIV1ProductRevisionHistoryItem,
	type BlendCalcAPIV1Source,
	type BlendCalcAPIV1SourceAttribution,
	type BlendCalcAPIV1Warning,
...
type BlendCalcAPIV1AssetSourceIdentity = {
	displayName: string;
	sourceUrl: string;
};

export type BlendCalcAPIV1SourceAttributionCatalog = {
	sources: Record<string, BlendCalcAPIV1SourceAttribution>;
	datasetsBySource: Record<
		string,
		Record<string, BlendCalcAPIV1SourceAttribution>
	>;
	datasetSourceKeys: Set<string>;
	assetSources: Record<string, BlendCalcAPIV1AssetSourceIdentity>;
};
...
const mapAssetSourceIdentity = (
	source: SourceAttributionRow,
): BlendCalcAPIV1AssetSourceIdentity | null => {
...
export const mapBlendCalcAPIV1SourceAttributionCatalog = (
	sourceRows: SourceAttributionRow[],
	datasetRows: DatasetAttributionRow[],
): BlendCalcAPIV1SourceAttributionCatalog => {
	const sources: BlendCalcAPIV1SourceAttributionCatalog["sources"] = {};
	const assetSources: BlendCalcAPIV1SourceAttributionCatalog["assetSources"] =
		{};
	...
	const datasetsBySource: BlendCalcAPIV1SourceAttributionCatalog["datasetsBySource"] =
		{};
...
const readBlendCalcAPIV1SourceAttributionCatalogFromDatabase = async (
	...
	return mapBlendCalcAPIV1SourceAttributionCatalog(
...
const selectSourceAttributions = (
	...
	catalog: BlendCalcAPIV1SourceAttributionCatalog,
) => {
	const attributions = new Map<string, BlendCalcAPIV1SourceAttribution>();
...
const toImage = (
	...
	assetSource: BlendCalcAPIV1AssetSourceIdentity,
): BlendCalcAPIV1Image => ({
	role: image.role,
	url: image.imageUrl,
...
const toWarning = (fact: FoodCompatibilityFact): BlendCalcAPIV1Warning => ({
	code: fact.slug,
	message: fact.label,
...
const toCanonicalFieldSource = (
	record: ApprovedCatalogRecord,
	fieldPath: string,
): BlendCalcAPIV1FieldSource | null => {
...
export const mapApprovedCatalogRecordToBlendCalcAPIV1Product = (
	record: ApprovedCatalogRecord,
	sourceAttributionCatalog: BlendCalcAPIV1SourceAttributionCatalog,
		...
		ingredients: {
			...
			structured: (record.food.structuredIngredients ?? []).map(
				function mapIngredient(
					ingredient,
				): BlendCalcAPIV1Product["ingredients"]["structured"][number] {
					return {
...
type RevisionHistoryRow =
	Database["public"]["Functions"]["get_blendcalc_api_product_revision_history_v1"]["Returns"][number];

const toRevisionChange = (
	value: unknown,
): BlendCalcAPIV1ProductRevisionChange | null => {
...
export const readBlendCalcAPIV1ProductRevisionHistory = async (
	supabase: SupabaseClient<Database>,
	barcodeValue: string,
