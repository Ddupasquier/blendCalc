...
const CATALOG_PRODUCT_REPAIR_ROUTE = "/profile/privileged-tools/data-operations";
...
type CatalogProductRepairActionEvent = Pick<RequestEvent, "locals" | "params" | "request">;
...
export const loadCatalogProductRepairWorkspace = async (
	{ locals, params }: CatalogProductRepairLoadEvent,
) => {
	...
	return {
		viewerRole: role,
		canRunRepairs: permissions.includes("data_operations.catalog_health.repair"),
...
export const runCatalogProductRepairAction = async (
	{ locals, params, request }: CatalogProductRepairActionEvent,
): Promise<CatalogHealthRepairActionData | ReturnType<typeof fail>> => {
	const productId = params.productId ?? "";
	if (!productId) throw new TypeError("Catalog product ID is required.");
	...
	const formData = await readLimitedFormData(request, CATALOG_REPAIR_FORM_MAX_BYTES);
	...
	if (
		...
		return fail(400, {
			...
			catalogRepairError: "Run a fresh safety check before applying a catalog repair.",
		});
	}
	...
	try {
		...
		if (!passport.issues.some((issue) =>
			...
			return fail(409, {
				catalogRepairOccurrenceKey: occurrenceKey,
				catalogRepairError: "This product's repair options have changed. Refresh it before trying again.",
			});
		}
		...
		if (result.status === "failed") {
			return fail(500, {
				catalogRepairOccurrenceKey: occurrenceKey,
				catalogRepairError: "The safety check stopped before making an uncertain change.",
			});
		}
		return {
			catalogRepairOccurrenceKey: occurrenceKey,
			...
			...(mode === "apply"
				? { catalogRepairSuccess: result.changedCount === 1
