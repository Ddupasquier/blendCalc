...
import { prepareNutritionLabelOcrImage } from "$lib/utils/food/images/selectedImagePreview.client";
...
export const runNutritionLabelOcrJob = async ({
	...
	const preparedImage = await prepareNutritionLabelOcrImage(
		file,
		crop,
		signal,
		NUTRITION_LABEL_OCR_JOB_MAX_INPUT_BYTES,
	...
	onPrepared?.();
	const preparedFile = new File([preparedImage], "nutrition-label.webp", {
		type: preparedImage.type || "image/webp",
	});
