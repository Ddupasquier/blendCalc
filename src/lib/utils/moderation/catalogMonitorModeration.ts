...
export const parseCatalogMonitorModerationSummary = (
	...
	return {
		settings: {
			...
			productBatchSize: number(
				settings.productBatchSize,
				"settings.productBatchSize",
			),
			safetyAlertPageSize: number(
				settings.safetyAlertPageSize,
				"settings.safetyAlertPageSize",
			),
			lastInvocationRequestedAt: nullableString(
				settings.lastInvocationRequestedAt,
				"settings.lastInvocationRequestedAt",
			),
			lastInvocationRequestId: nullableNumber(
				settings.lastInvocationRequestId,
				"settings.lastInvocationRequestId",
			),
			lastInvocationError: nullableString(
				settings.lastInvocationError,
				"settings.lastInvocationError",
			),
		},
		queue: {
			...
			retryingProducts: number(
				queue.retryingProducts,
				"queue.retryingProducts",
			),
			pendingProviderChanges: number(
				queue.pendingProviderChanges,
				"queue.pendingProviderChanges",
			),
			pendingSafetyMatches: number(
				queue.pendingSafetyMatches,
				"queue.pendingSafetyMatches",
			),
			activeSafetyMatches: number(
				queue.activeSafetyMatches,
				"queue.activeSafetyMatches",
			),
		},
		recentRuns: array(root.recentRuns, "recentRuns").map((value, index) => {
			...
			return {
				...
				productJobsClaimed: number(
					run.productJobsClaimed,
					`${path}.productJobsClaimed`,
				),
				productJobsChanged: number(
					run.productJobsChanged,
					`${path}.productJobsChanged`,
				),
				productJobsFailed: number(
					run.productJobsFailed,
					`${path}.productJobsFailed`,
				),
				safetyAlertsObserved: number(
					run.safetyAlertsObserved,
					`${path}.safetyAlertsObserved`,
				),
				safetyAlertsChanged: number(
					run.safetyAlertsChanged,
					`${path}.safetyAlertsChanged`,
				),
				safetyMatchesActivated: number(
					run.safetyMatchesActivated,
					`${path}.safetyMatchesActivated`,
				),
				errors: array(run.errors, `${path}.errors`),
			};
		}),
		providerChanges: array(root.providerChanges, "providerChanges").map(
			(value, index) => {
				...
				return {
					...
					changeSummary: {
						changes: array(
							summary.changes,
							`${path}.changeSummary.changes`,
						).map((value, changeIndex) => {
					...
					materialFieldPaths: array(
						change.materialFieldPaths,
						`${path}.materialFieldPaths`,
					).map((entry, entryIndex) =>
						string(entry, `${path}.materialFieldPaths[${entryIndex}]`),
					),
		...
		safetyMatches: array(root.safetyMatches, "safetyMatches").map(
			(value, index) => {
				...
				return {
					...
					codeInformation: nullableString(
