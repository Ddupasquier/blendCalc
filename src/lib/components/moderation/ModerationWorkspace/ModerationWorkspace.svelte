<script lang="ts">
	...
	const getImageCrop = (
		submission: ModerationWorkspaceProps["data"]["productSubmissions"][number],
	...
	const hasAccountModerationAction = (
		user: ModerationWorkspaceProps["data"]["users"][number],
	) => (
	...
	const formatAvatarReviewStatus = (value: string) => {
		if (value === "self_attested") return "Waiting for review";
		...
		scope === "profile-images"
			? data.users.filter(
					(user) =>
						...
						user.avatarModerationStatus === "self_attested",
				)
			: data.users,
	);
	...
	const submitAccountSearch = async (event: SubmitEvent) => {
		...
		const href = query
				? scope === "all"
					...
					: `/profile/moderator-actions/${scope}?q=${encodeURIComponent(query)}`
				: scope === "all"
					? "/moderation"
					: `/profile/moderator-actions/${scope}`;
...
<div class="moderation-page" data-scope={scope}>
	{#if scope === "all"}
		<header>
			<p class="eyebrow">{data.viewerRole}</p>
			<h1>Moderation</h1>
			<p>Review community reports, shared products, profile images, and account access.</p>
			<a class="data-health-link" href="/profile/moderator-actions/catalog-data-health">Review catalog data health</a>
		</header>
	{/if}

	{#if scope === "all" || scope === "account-access" || scope === "profile-images"}
	<section class="account-search" aria-labelledby="account-search-title">
		<div>
				<h2 id="account-search-title">
					{scope === "profile-images" ? "Find a profile image" : "Find an account"}
				</h2>
				<p>
					{scope === "profile-images"
						? "Search images that still need a moderation decision."
						: "Search by preferred name, email address, user ID, role, or account status."}
				</p>
		</div>
		<form method="GET" role="search" onsubmit={submitAccountSearch}>
			...
			<div class="search-controls">
				...
				{#if data.query}
					<a
						class="clear-search"
						href={scope === "all" ? "/moderation" : `/profile/moderator-actions/${scope}`}
					>Clear</a>
				{/if}
			</div>
		</form>
		<p class="result-count" aria-live="polite">
			{#if data.query}
					{visibleUsers.length} of {data.totalCount} accounts shown
			{:else}
					{scope === "profile-images"
						? `${visibleUsers.length} images waiting for review`
	...
	<section
		...
		aria-labelledby={scope === "all" ? "product-review-title" : undefined}
		aria-label={scope === "all" ? undefined : "Product submissions"}
	>
		{#if scope === "all"}
		<div>
				...
				<article class="product-card">
					<header>
						...
						<div class="product-card__statuses">
							...
							{/if}
								<span class="status">Needs review</span>
					...
					<dl>
						...
						<div>
								<dt>Matched food record</dt>
								<dd>{submission.matchedSource ?? "No exact source match"}</dd>
						</div>
							<div><dt>Package photos</dt><dd>{submission.evidenceComplete ? "Ready to review" : "Missing required photos"}</dd></div>
							<div><dt>Differences to review</dt><dd>{submission.conflictCount}</dd></div>
					</dl>
					{#if submission.updateReview}
	...
	<section
		...
		class="compatibility-review"
		aria-labelledby={scope === "all" ? "compatibility-review-title" : undefined}
		aria-label={scope === "all" ? undefined : "Food warning reports"}
	>
		{#if scope === "all"}
		<header>
			<div>
				...
				<article class="compatibility-review__card">
					...
					<dl>
						{#if feedback.feedbackType === "missing_warning"}
							...
							<div>
								<dt>Setting type</dt>
								<dd>{feedback.preferenceType ? formatReadableLabel(feedback.preferenceType) : "Unknown"}</dd>
							</div>
						{:else}
							<div><dt>Warning</dt><dd>{feedback.issueCode ? formatReadableLabel(feedback.issueCode) : "Unknown warning"}</dd></div>
						{/if}
						<div><dt>Reason</dt><dd>{formatReadableLabel(feedback.reportReason)}</dd></div>
	...
	<section id="account-review" class="account-list" aria-label="User accounts">
		{#each visibleUsers as user (user.id)}
			<article class:account-card--blocked={user.status === "banned"} class="account-card">
				...
				<div class="account-details">
					...
					<span class="status">{formatReadableLabel(user.status)}</span>
						{#if hasAccountModerationAction(user)}
							<PrivilegedActionBadge />
						{/if}
					</div>
					<p class="account-email">{user.email}</p>
				<p>Profile image: {formatAvatarReviewStatus(user.avatarModerationStatus)}</p>
				{#if user.role}<p>Role: {formatReadableLabel(user.role)}</p>{/if}
		...
		{:else}
			<div class="empty-results">
				<strong>{scope === "profile-images" ? "No profile images need review" : "No accounts found"}</strong>
				<p>
					{scope === "profile-images"
						? "New self-attested profile images will appear here."
						: "Try a different preferred name, email address, user ID, role, or status."}
				</p>
			</div>
		{/each}
	</section>
