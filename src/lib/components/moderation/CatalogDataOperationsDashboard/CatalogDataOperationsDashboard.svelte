<script lang="ts">
	...
	let { dashboard, catalogMonitor }: CatalogDataOperationsDashboardProps = $props();
	...
	const operationalOverviewItems = $derived([
		...
		{
			label: "Products ready for API v1",
...
<section class="catalog-data-operations" aria-label="Catalog data operations">
	...
	<section class="catalog-data-operations__overview" aria-label="Operational overview">
	...
	<div class="catalog-data-operations__sections">
		...
		>
			<div class="catalog-data-operations__stack">
				<article class="catalog-data-operations__record">
					<header>
						<div>
							...
							<span>Product changes and official safety notices are collected without silently replacing catalog facts.</span>
						</div>
						<TextBadge
				...
				{:else}
					<p class="catalog-data-operations__empty">No monitoring run has been recorded yet.</p>
				{/each}
			</div>
		</CollapsibleSection>
		...
		<CollapsibleSection title="Source activity" badge={`${dashboard.sources.length}`} surface="panel">
			<div class="catalog-data-operations__stack">
				<p class="catalog-data-operations__muted">
					Most-used sources appear first, based on lookups during this {dashboard.metricWindowDays}-day window.
				</p>
					...
					<article class="catalog-data-operations__record">
						<header>
							<div>
								...
								<span>{source.enabled ? "Available for lookups" : "Not used for lookups"}</span>
							</div>
						...
						{#if source.policyIssues.length > 0}
							<ul class="catalog-data-operations__issues">
								{#each source.policyIssues as issue}<li>{getCatalogIssueReasonLabel(issue)}</li>{/each}
							</ul>
						{/if}
						{#if source.latestEvaluation}
							<div class="catalog-data-operations__evaluation">
								<strong>Latest evaluation: {getCatalogHealthStatusLabel(source.latestEvaluation.decision)}</strong>
				...
				{:else}
					<p class="catalog-data-operations__empty">No source activity is available.</p>
				{/each}
			</div>
		</CollapsibleSection>

		<CollapsibleSection title="Dataset imports and licensing" badge={`${dashboard.datasets.length}`} surface="panel">
			<div class="catalog-data-operations__stack">
				{#each dashboard.datasets as dataset (dataset.key)}
					<article class="catalog-data-operations__record">
						<header>
							...
							<TextBadge label={getCatalogHealthStatusLabel(dataset.licenseReviewStatus)} tone={dataset.licenseReviewStatus === "approved" ? "success" : "neutral"} />
						</header>
						<dl class="catalog-data-operations__metrics">
						...
						{#if dataset.issues.length > 0}
							<ul class="catalog-data-operations__issues">
								{#each dataset.issues as issue}<li>{getCatalogIssueReasonLabel(issue)}</li>{/each}
							</ul>
						{/if}
					</article>
				{/each}
