 ...
 * Do not run directly.
 * Parent workflow: `npm run audit:api-catalog`
 */
...
export const isCatalogReadinessContractUnavailable = (error) => {
	...
	const message = "message" in error && typeof error.message === "string"
		...
		: "";
	return ROLLOUT_MISSING_CONTRACT_CODES.has(code)
...
export const classifyCatalogReadinessIssue = (issueContract) => {
	...
	return READINESS_OWNER_BY_RESPONSIBLE_GROUP[
		issueContract.responsible_group
	] ?? "unclassified";
};
...
export const buildCatalogReadinessSummary = (report) => {
	...
	/** @param {string} owner */
	const countWithOwner = (owner) => withheldRows.filter((row) =>
	...
	return {
		...
		withheldNeedingDataOperationsReview: countWithOwner("data_operations_review"),
		...
		withheldWithoutIssueContract: withheldRows.filter((row) =>
			row.issues.length === 0 || row.issues.some((issue) => issue.owner === "unclassified")
		).length,
		withholdingReasonCounts: countValues(withheldRows.flatMap((row) => row.reasons)),
		issueCodeCounts: countValues(withheldRows.flatMap((row) =>
			row.issues.map((issue) => issue.issueCode)
		)),
