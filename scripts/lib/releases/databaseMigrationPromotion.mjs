...
const runGit = (argumentsList, repositoryRoot) =>
...
export const refreshRemoteMainReference = ({
	...
	runGitCommand(
		["rev-parse", "--verify", DEFAULT_MAIN_REFERENCE],
		repositoryRoot,
	);
};
...
export const findMigrationsNotIdenticalToRemoteMain = ({
	...
	const migrationFileNames = readdirSync(migrationsDirectoryPath)
		...
		.sort();
	const readRemoteMigration =
		readPromotedMigration ??
		((relativePath) => {
			try {
				return runGit(
					["show", `${mainReference}:${relativePath}`],
					repositoryRoot,
				);
			} catch {
				return null;
	...
	return migrationFileNames.flatMap((fileName) => {
		const relativePath = `${migrationsDirectory}/${fileName}`;
