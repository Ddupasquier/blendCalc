...
const requestedQueryCount = Number(
	process.argv
		.find((argument) => argument.startsWith("--queries="))
		?.split("=")[1] ?? 100,
);
...
const readAllRows = async (table, columns, configure = (query) => query) => {
	...
	for (let offset = 0; ; offset += 1000) {
		const query = configure(supabase.from(table).select(columns)).range(
			offset,
			offset + 999,
		);
...
const createSearchQueryCorpus = (records, datasetKeys) => {
	...
	const selectedQueries = datasetKeys.flatMap((datasetKey) =>
		rankedCandidates
			.filter(
				(candidate) =>
					candidate.datasetKeys.size === 1 &&
					candidate.datasetKeys.has(datasetKey),
			)
...
const main = async () => {
	const [datasets, records, identifiers] = await Promise.all([
		readAllRows(
			...
			(query) =>
				query.eq("active", true).eq("import_enabled", true).order("key"),
		),
		readAllRows(
	...
	const activeRecords = records.filter((record) =>
		activeDatasetKeys.has(record.dataset_key),
	);
	...
	for (const record of activeRecords) {
		...
		const datasetKeys =
			normalizedDescriptionDatasets.get(normalized) ?? new Set();
	...
	for (const identifier of activeIdentifiers) {
		...
		exactIdentifierRecordKeys
			.get(identifier.dataset_key)
			?.add(`${identifier.dataset_key}\u0000${identifier.source_food_key}`);
	}
	...
	for (const search of searchRun.values) {
		const resultDatasetKeys = new Set(
			search.results.map((result) => result.dataset_key),
		);
	...
	const report = {
		...
		method: {
			...
			queryConstruction:
				"A balanced corpus of leading two-word phrases: half source-exclusive candidates and half phrases represented across datasets.",
			identityBoundary:
				"Exact source identifiers are identity evidence. Normalized description and search overlap are contribution metrics only.",
		},
	...
	console.log(
		`Generic dataset contribution audit (${searchQueries.length} queries)`,
	);
