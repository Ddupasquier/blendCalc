...
const authenticatedBrowserStateDirectory = new URL(
...
const getLocalEnvironment = () => {
	...
	if (!apiUrl || !publishableKey || !serviceRoleKey) {
		throw new Error(
			"The local Supabase status did not provide the required API keys.",
		);
	}
	...
	if (hostname !== "127.0.0.1" && hostname !== "localhost") {
		throw new Error(
			`Refusing to treat non-local Supabase URL ${apiUrl} as a test database.`,
		);
	}
...
const writeTestEnvironment = async (environment) => {
	const accountByKey = new Map(
		testAccounts.map((account) => [account.key, account]),
	);
	const accountSummaries = testAccounts.map(
		({ key, displayName, email, role, purpose }) => ({
			key,
			displayName,
			email,
			role,
	...
	const lines = [
		...
		`BLENDCALC_TEST_ACCOUNTS_BASE64=${Buffer.from(JSON.stringify(accountSummaries)).toString("base64")}`,
		"",
	];
...
const createQaGoals = (goals, pointGoalTolerance) =>
	Object.fromEntries(
		Object.entries(goals).map(([nutrientId, targetAmount], index) => {
			...
			const goalType = [1003, 1079].includes(parsedNutrientId)
				? "minimum"
				: parsedNutrientId === 1004
					? "maximum"
					: "exact";
			return [
				parsedNutrientId,
				{
					nutrientId: parsedNutrientId,
					goalType,
					targetAmount,
					upperAmount: null,
					toleranceRatio: pointGoalTolerance,
					importanceWeight: 1,
					sortOrder: index + 1,
				},
			];
...
const createSavedRecipe = (
	...
	return {
		...
		nutrientGoals: createQaGoals(definition.goals, pointGoalTolerance),
		goalBasis: "per_mix",
		servingGrams,
		servingQuantities,
		servingUnits,
...
const seedTestMixPreferences = async (
	userClient,
	account,
	catalogFoods,
	pointGoalTolerance,
...
const seedTestAccountState = async (
	...
	const pointGoalToleranceRow = requireSuccessfulResult(
		await userClient
			.from("mix_runtime_configuration")
			.select("value")
			.eq("key", "point-goal-tolerance")
			.eq("enabled", true)
			.single(),
		`Load Mix goal tolerance for ${account.email}`,
	);
	const pointGoalTolerance = Number(pointGoalToleranceRow.value);
	if (!Number.isFinite(pointGoalTolerance) || pointGoalTolerance < 0) {
		throw new Error(`Mix goal tolerance is invalid for ${account.email}.`);
	}
...
const seedModerationFixtures = async (admin, submittingUserId) => {
	if (!submittingUserId) {
		throw new Error(
			"The populated QA user is unavailable for moderation fixtures.",
		);
	}
...
const seedTestAccounts = async (environment) => {
	...
	for (const account of testAccounts) {
		...
		await seedTestAccountState(environment, account, user.id, catalogFoods);
...
const startLocalStack = async ({
	...
	runCommand(
		"supabase",
		["start", "--exclude", "edge-runtime,logflare,vector"],
		{ capture: true },
	);
	if (applyMigrations) {
...
const resetLocalStack = async () => {
	...
	await rm(authenticatedBrowserStateDirectory, {
		force: true,
		recursive: true,
	});
...
const main = async () => {
	if (action === "verify") assertLocalResourceSafety();
