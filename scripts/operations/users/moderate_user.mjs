/**
 * Purpose: Perform privileged local user operations by email: grant/revoke moderator,
 * admin, or developer roles, ban an account, or reverse a ban while recording
 * moderation audit rows.
 * This writes Auth and moderation tables and requires service-role credentials.
 * Role example: `node scripts/operations/users/moderate_user.mjs role user@example.com developer --user-id=<uuid>`
 * Ban example: `node scripts/operations/users/moderate_user.mjs ban user@example.com terms_violation`
 */
...
const [command, rawEmail, value, ...options] = process.argv.slice(2);
const email = rawEmail?.trim().toLowerCase();
const expectedUserId = options
	.find((option) => option.startsWith("--user-id="))
	?.slice("--user-id=".length)
	.trim();

const usage = () => {
	console.error(`Usage:
  node scripts/operations/users/moderate_user.mjs role user@example.com moderator
  node scripts/operations/users/moderate_user.mjs role user@example.com admin
  node scripts/operations/users/moderate_user.mjs role user@example.com developer --user-id=<uuid>
  node scripts/operations/users/moderate_user.mjs role user@example.com none
...
const setRole = async (user) => {
	if (!new Set(["moderator", "admin", "developer", "none"]).has(value)) {
		throw new Error("Role must be moderator, admin, developer, or none.");
	}
	if (expectedUserId && user.id !== expectedUserId) {
		throw new Error(
			`The account ID for ${email} is ${user.id}, not ${expectedUserId}.`,
		);
	}
...
const banUser = async (user) => {
	...
	console.log(
		`Blocked ${email} from the app and future signup with that email.`,
	);
};
