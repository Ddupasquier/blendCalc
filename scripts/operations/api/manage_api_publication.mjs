/**
 * Purpose: Review API correction/rights concerns and place, release, or resolve
 ...
 * Run: `npm run api:publication -- list`
 * List: `npm run api:publication -- list`
 * Hold: `npm run api:publication -- hold product 00021130493609 rights-review --actor-email=moderator@example.com --public-message="Temporarily unavailable while publication rights are reviewed." --internal-note="Rights-holder report received."`
 * Release: `npm run api:publication -- release <hold-uuid> --actor-email=moderator@example.com --note="Evidence reviewed; publication may resume."`
 * Resolve: `npm run api:publication -- resolve <concern-uuid> resolved publication-hold --actor-email=moderator@example.com --note="Placed a publication hold."`
 */
...
const [command, ...positionals] = process.argv.slice(2).filter(
...
const usage = () => {
	console.error(`Usage:
  npm run api:publication -- list
  npm run api:publication -- hold <product|image|dataset|source> <reference> <reason> --actor-email=<email> --public-message=<message> --internal-note=<note> [--concern-id=<uuid>]
  npm run api:publication -- release <hold-uuid> --actor-email=<email> --note=<note>
  npm run api:publication -- resolve <concern-uuid> <resolved|dismissed> <action> --actor-email=<email> --note=<note>
...
const getActor = async () => {
	...
	if (!email) throw new Error("Provide --actor-email=<elevated-account-email>.");
	for (let page = 1; page <= 100; page += 1) {
		...
		if (user) {
			...
			if (!role) throw new Error(`${email} does not have an elevated application role.`);
			return user;
		}
...
const resolveSubject = async (subjectType, reference) => {
	...
	const normalizedReference = subjectType === "product"
		? normalizeGtin(reference)
		: reference.trim();
...
const listQueue = async () => {
	const [{ data: concerns, error: concernError }, { data: holds, error: holdError }] =
		await Promise.all([
			supabase.from("api_publication_concerns")
				.select("id, urgency, reporter_type, concern_type, subject_type, subject_reference, status, created_at")
				.in("status", ["open", "triaged"])
				.order("urgency", { ascending: false })
				.order("created_at", { ascending: true })
				.limit(100),
			supabase.from("api_publication_holds")
...
const placeHold = async () => {
	...
	if (!SUBJECT_TYPES.has(subjectType) || !HOLD_REASONS.has(reasonCode) || !reference) {
		usage();
		process.exitCode = 1;
		return;
	}
	...
	const readExistingHold = () => supabase.from("api_publication_holds")
		.select("id")
		.eq("subject_type", subjectType)
		.eq(subject.targetColumn, subject.targetValue)
		.is("released_at", null)
		.maybeSingle();
	const { data: existingHold, error: existingHoldError } = await readExistingHold();
	if (existingHoldError) throw existingHoldError;
	if (existingHold) {
		console.log(`Publication hold ${existingHold.id} is already active on ${subjectType} ${subject.normalizedReference}.`);
		return;
	}
	const { data, error } = await supabase.from("api_publication_holds").insert({
	...
	if (error?.code === "23505") {
		const { data: concurrentHold, error: concurrentHoldError } = await readExistingHold();
		if (concurrentHoldError) throw concurrentHoldError;
		if (concurrentHold) {
			console.log(`Publication hold ${concurrentHold.id} is already active on ${subjectType} ${subject.normalizedReference}.`);
			return;
		}
	}
	if (error) throw error;
	console.log(`Placed publication hold ${data.id} on ${subjectType} ${subject.normalizedReference}.`);
};

const releaseHold = async () => {
	const [holdId] = positionals;
	const note = options.get("note")?.trim();
	if (!UUID_PATTERN.test(holdId ?? "") || !note) throw new Error("Provide a valid hold UUID and --note.");
	const actor = await getActor();
	const { data, error } = await supabase.from("api_publication_holds")
		.update({ released_by: actor.id, released_at: new Date().toISOString(), release_note: note })
		.eq("id", holdId).is("released_at", null).select("id").maybeSingle();
	if (error) throw error;
...
const resolveConcern = async () => {
	...
	const note = options.get("note")?.trim();
	if (!UUID_PATTERN.test(concernId ?? "") || !new Set(["resolved", "dismissed"]).has(status) || !RESOLUTION_ACTIONS.has(resolutionAction) || !note) {
		usage();
		process.exitCode = 1;
		return;
	}
	if (status === "dismissed" && resolutionAction !== "no-change") {
		throw new Error("Dismissed concerns must use the no-change resolution action.");
	}
	const actor = await getActor();
	const { data, error } = await supabase.from("api_publication_concerns").update({
		status,
		resolution_action: resolutionAction,
		resolution_note: note,
		reviewed_by: actor.id,
		reviewed_at: new Date().toISOString(),
	}).eq("id", concernId).in("status", ["open", "triaged"]).select("id").maybeSingle();
	if (error) throw error;
	if (!data) throw new Error("No unresolved concern matched that UUID.");
	console.log(`${status === "resolved" ? "Resolved" : "Dismissed"} publication concern ${concernId}.`);
};
