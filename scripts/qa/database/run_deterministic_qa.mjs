...
const assertDatasetCatalog = async () => {
	const datasets = requireData(
		await admin
			...
			.select(
				"key, active, import_enabled, license_review_status, source_url, license_url, attribution_text, imported_at, source_file_sha256, food_count, nutrient_value_count, measure_count",
			)
	...
	assert(
		cnf?.active && cnf.import_enabled,
		"CNF 2026 is not active/import-enabled.",
	);
	assert(
		cnf.license_review_status === "approved",
		"CNF 2026 is not legally approved.",
	);
	assert(
		cnf.food_count === 5993,
		`CNF food count is ${cnf.food_count}, expected 5993.`,
	);
	assert(
		cnf.nutrient_value_count === 565409,
		`CNF nutrient count is ${cnf.nutrient_value_count}, expected 565409.`,
	);
	assert(
		cnf.measure_count === 29867,
		`CNF measure count is ${cnf.measure_count}, expected 29867.`,
	);

	assert(
		cofid?.active && cofid.import_enabled,
		"CoFID 2021 is not active/import-enabled.",
	);
	assert(
		cofid.license_review_status === "approved",
		"CoFID 2021 is not legally approved.",
	);
	assert(
		cofid.food_count === 2887,
		`CoFID food count is ${cofid.food_count}, expected 2887.`,
	);
	assert(
		cofid.nutrient_value_count === 199415,
		`CoFID nutrient count is ${cofid.nutrient_value_count}, expected 199415.`,
	);

	for (const dataset of [cnf, cofid]) {
		assert(
			dataset.source_url?.startsWith("https://"),
			`${dataset.key} lacks a source URL.`,
		);
		assert(
			dataset.license_url?.startsWith("https://"),
			`${dataset.key} lacks a license URL.`,
		);
		assert(
			dataset.attribution_text?.trim(),
			`${dataset.key} lacks attribution.`,
		);
		assert(dataset.imported_at, `${dataset.key} lacks an import timestamp.`);
		assert(
			/^[a-f0-9]{64}$/.test(dataset.source_file_sha256 ?? ""),
			`${dataset.key} lacks a valid file hash.`,
		);
	}
	...
	const sourcePolicies = requireData(
		await admin
			...
			.select(
				"key, terms_url, attribution_text, canonical_storage_allowed, canonical_license_name, canonical_policy_reviewed_at, canonical_policy_notes, provenance",
			)
	...
	for (const [sourceKey, expectedLicense] of [
		...
		assert(
			...
			`${sourceKey} canonical storage is not approved.`,
		);
		assert(
			source.canonical_license_name === expectedLicense,
			`${sourceKey} has the wrong canonical licence.`,
		);
		assert(
			source.terms_url?.startsWith("https://"),
			`${sourceKey} lacks a licence URL.`,
		);
		assert(
			source.attribution_text?.trim(),
			`${sourceKey} lacks required attribution text.`,
		);
		assert(
			source.canonical_policy_reviewed_at,
			`${sourceKey} lacks a policy review date.`,
		);
		assert(
			/attribution/i.test(source.canonical_policy_notes ?? ""),
			`${sourceKey} policy notes omit attribution.`,
		);
		assert(
			/third-party rights/i.test(source.canonical_policy_notes ?? ""),
			`${sourceKey} policy notes omit excluded rights.`,
		);
		assert(
			source.provenance?.canonical_policy_review?.requires_attribution === true,
			`${sourceKey} attribution requirement is not machine-readable.`,
		);
		assert(
			source.provenance?.canonical_policy_review?.public_api_eligible === true,
			`${sourceKey} is not marked API-eligible.`,
		);
	}
	...
	for (const [datasetKey, searchTerm] of [
		...
		assert(
			result,
			`${datasetKey} did not return an attributed QA search result.`,
		);
		assert(
			result.source_url?.startsWith("https://"),
			`${datasetKey} response lacks its source URL.`,
		);
		assert(
			result.license_name?.trim(),
			`${datasetKey} response lacks its licence name.`,
		);
		assert(
			result.license_url?.startsWith("https://"),
			`${datasetKey} response lacks its licence URL.`,
		);
		assert(
			result.attribution_text?.trim(),
			`${datasetKey} response lacks attribution text.`,
		);
	}

	assert(
		afcd?.license_review_status === "requires_acceptance",
		"AFCD legal gate changed unexpectedly.",
	);
	assert(
		!afcd.import_enabled && !afcd.active,
		"AFCD must stay disabled before terms acceptance.",
	);
	...
	assert(
		profiles.length > 0,
		"No enabled nutrition completeness profiles exist.",
	);
	...
	pass(
		"QA-053-001 dataset metadata, redistribution policy, attribution, and DB-owned completeness policy",
	);
};
...
const assertSourceRegistry = async () => {
	...
	assert(
		foodRepo.provenance?.retired_on === "2026-02-28",
		"FoodRepo retirement date is missing.",
	);
	assert(
		foodRepo.provenance?.replacement_source_key === "open-food-facts",
		"FoodRepo replacement is missing.",
	);
	assert(
		ocr?.provenance?.requires_user_confirmation === true,
		"OCR must remain confirmation-only.",
	);
	assert(
		gs1?.provenance?.role === "product_identifier_carrier",
		"GS1 role is incorrect.",
	);
	assert(
		gs1.provenance?.network_policy === "never_fetch_arbitrary_scanned_urls",
		"GS1 network policy is unsafe.",
	);
	...
	assert(
		evaluation?.decision === "retired",
		"FoodRepo evaluation is not retired.",
	);
	assert(
		evaluation.sample_size === 0,
		"FoodRepo should not claim a completed benchmark.",
	);
	assert(
		/not run|not enter/i.test(evaluation.summary),
		"FoodRepo evaluation does not explain the skipped benchmark.",
	);
	assert(
		evaluation.details?.planned_sample_size === 200,
		"FoodRepo planned sample size is not recorded.",
	);
...
const assertMetricsPrivacy = async () => {
	...
	assert(
		publicRead.error,
		"Publishable-key client can read service-only source metrics.",
	);
...
const assertCatalogReads = async () => {
	...
	for (const barcode of ["00021130493609", "00021130462506"]) {
		const rows = requireData(
			await admin.rpc("get_blendcalc_api_product_v1", { p_barcode: barcode }),
			`catalog product ${barcode}`,
		);
		assert(
			rows.length === 1,
			`Active catalog product ${barcode} was not found.`,
		);
	...
	const pastaImages = requireData(
		await admin
			...
			.select(
				"id, image_url, thumbnail_url, license_name, attribution_text, fit_mode, placement_version",
			)
			.eq("shared_product_id", pastaSauce.id)
			.eq("status", "active"),
		"pasta sauce images",
	...
	assert(
		pastaImages.length > 0,
		"Pasta sauce has no active approved image metadata.",
	);
	for (const image of pastaImages) {
		assert(
			image.image_url?.startsWith("http"),
			"An active image lacks a usable URL.",
		);
		assert(
			image.license_name?.trim(),
			"An active image lacks license metadata.",
		);
	}
	...
	assert(
		jellyImages.length === 0,
		"Jelly unexpectedly has an active image; update its QA fixture.",
	);

	const firstPage = requireData(
		await admin.rpc("search_blendcalc_api_products_v1", {
			p_query: "tomato",
			p_terms: ["tomato"],
			p_limit: 15,
			p_offset: 0,
	...
	assert(
		firstPage.length <= 15,
		"Catalog search exceeded its requested page size.",
	);
	...
	assert(
		secondPage.length === 0 ||
			Number(secondPage[0]?.total_count ?? 0) === total,
		"Catalog search totals changed between pages.",
	);
	const firstIds = new Set(firstPage.map((product) => product.id));
	assert(
		!secondPage.some((product) => firstIds.has(product.id)),
		"Catalog search pages contain duplicate IDs.",
	);
	const nameMatchPosition = (name) =>
		String(name).toLowerCase().indexOf("tomato");
	const positions = firstPage.map((product) =>
		nameMatchPosition(product.product_name),
	);
	...
	assert(
		privateSearch.length === 0,
		"An account-only QA ingredient leaked into the canonical catalog.",
	);
	...
	assert(
		...
		"Stored blendCalc catalog reads changed provider API metrics.",
	);
	pass(
		"QA-058-001 through QA-058-003 and QA-058-005 through QA-058-006 catalog data invariants",
	);
};
...
const assertOpenApiContract = async () => {
	...
	for (const path of paths) {
		assert(
			Object.keys(specification.paths[path]).every(
				(method) => method === "get",
			),
	...
	assert(
		...
		"blendCalcAPI v1 products do not require source attribution metadata.",
	);
	assert(
		productSchema?.properties?.sourceAttributions?.items?.$ref ===
			"#/components/schemas/SourceAttribution",
		"blendCalcAPI v1 source attribution does not use the documented schema.",
	);
	assert(
		specification.security?.[0]?.cookieAuth,
		"OpenAPI does not require the app session.",
	);
...
const assertGenericFoodSourceFidelity = async () => {
	...
	assert(
		abiyuch?.description === "Abiyuch, raw",
		"CNF Abiyuch identity changed.",
	);
	assert(
		abiyuch.source_display_name?.trim(),
		"CNF Abiyuch lacks source attribution.",
	);
	assert(
		Array.isArray(abiyuch.nutrients) && abiyuch.nutrients.length > 0,
		"CNF Abiyuch lacks nutrients.",
	);
	assert(
		Array.isArray(abiyuch.measures) && abiyuch.measures.length > 0,
		"CNF Abiyuch lacks measures.",
	);
	...
	const arrowroot = arrowrootRows.find(
		(row) =>
			row.dataset_key === "cofid-2021" && row.source_food_key === "11-001",
	);
	assert(
		arrowroot?.description === "Arrowroot",
		"CoFID Arrowroot identity changed.",
	);
	assert(
		Array.isArray(arrowroot.nutrients) && arrowroot.nutrients.length > 0,
		"CoFID Arrowroot lacks nutrients.",
	);
	...
	const traceRows = requireData(
		await admin
			...
			.select(
				"source_nutrient_name, amount_per_100g, value_status, nutrient_id, mapping_status",
			)
	...
	for (const nutrientName of [
		...
		const row = traceRows.find(
			(candidate) => candidate.source_nutrient_name === nutrientName,
		);
		assert(row, `CoFID Arrowroot is missing trace nutrient ${nutrientName}.`);
		assert(
			row.amount_per_100g === null,
			`${nutrientName} trace was converted to numeric zero.`,
		);
		assert(
			row.nutrient_id !== null && row.mapping_status === "canonical",
			`${nutrientName} trace lost its canonical mapping.`,
		);
	}
