...
const getFdcCategoryValues = (foodCategory) => {
	...
	return [foodCategory.description, foodCategory.code, foodCategory.type].filter(Boolean);
};
...
const readAllActiveProducts = async () => {
	const { data, error } = await supabase
		...
		.select("id, barcode, product_name, approved_submission_id, category_option_id, food")
		.eq("status", "active")
		.order("barcode");
...
const readApprovedSubmissions = async (products) => {
	const ids = products.map((product) => product.approved_submission_id).filter(Boolean);
...
const saveCategoryObservations = async (barcode, observations) => {
	...
	const { data: existing, error: existingError } = await supabase
		...
		.select("source, source_field, normalized_value, source_reference, first_seen_at, observation_count")
		.eq("query", barcode);
	if (existingError) throw existingError;
	const existingByKey = new Map(
		(existing ?? []).map((row) => [
			[row.source, row.source_field, row.normalized_value, row.source_reference].join("|"),
			row,
		]),
	);
...
const rebuildExactMappings = async (normalizedValues) => {
	...
	const { data: options, error: optionError } = await supabase
		...
		.select("id, label, normalized_value, sources, source_count, observation_count, first_seen_at, last_seen_at")
		.in("normalized_value", normalizedValues);
	if (optionError) throw optionError;
	const { data: observations, error: observationError } = await supabase
		.from("custom_food_category_observations")
		.select("normalized_value, source, source_field, source_value, observation_count, first_seen_at, last_seen_at")
		.in("normalized_value", normalizedValues);
	...
	const rows = (options ?? []).map((option) => {
		...
		return {
			...
			source_fields: [...new Set(matching.map((item) => item.source_field))].sort(),
...
const updateCatalogRows = async ({ product, submission, resolved, sourceValues }) => {
	if (isDryRun) return;
...
const main = async () => {
	...
	await rebuildExactMappings(
		[...new Set(allObservedValues.map(normalizeFoodCategoryValue).filter(Boolean))],
	);
	...
	console.log(
		JSON.stringify({ scanned: products.length, updated, unresolved, dryRun: isDryRun }, null, 2),
	);
};
