...
| ------------------------------------------ | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| USDA FoodData Central                      | Exact GTIN, source food id/type, names, brand, ingredients, numeric nutrients, units, serving size, household serving, package weight, market country, publication/availability/update/discontinued dates, and categories. USDA documentation distinguishes reported zero, missing, and label statements such as `not a significant source`; the current runtime adapter maps numeric fields and preserves reviewed label statements when supplied as evidence.                                                                              | `src/lib/server/products/sources/usdaBarcodeProduct.server.ts`                                                                                                    |
| Open Food Facts                            | Exact GTIN, names, brand, source ingredient text and recursive structured ingredients, ingredient analysis, additives, explicit allergens, explicit traces, labels, categories, nutrients and explicit nutrient modifiers such as `<`, serving text/weight/volume, package quantity, package images, language, market countries, record/schema revisions, source timestamps, completeness, quality/obsolete state, and tag-source metadata. Raw responses stay private; client/storage ingredient text uses the shared versioned normalizer. | `src/lib/server/products/sources/openFoodFactsBarcodeProduct.server.ts`                                                                                           |
...
| User nutrition-label OCR                   | Text, exact nutrient candidates, explicit `<` limits, and `not a significant source` statements from a user-provided label                                                                                                                                                                                                                                                                                                                                                                                                                   | Tesseract runs on the client; no value or statement is accepted until the user confirms it; shared-submission images remain private evidence                      |
...
- **Package information:** raw private source evidence, normalized canonical ingredient
  text and lists, structured ingredients, ingredient percentages and
...
Provider capability is not assumed globally. Open Food Facts modifiers are accepted
only when the payload explicitly identifies a serving or 100 g basis. USDA omissions
