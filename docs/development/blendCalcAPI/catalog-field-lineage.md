...
Every product returned by blendCalcAPI v1 starts with one active row in `shared_products`.
...
1. A trusted server service-role client calls `get_blendcalc_api_product_v1` or
   `search_blendcalc_api_products_v1` to select an active `shared_products` row that passes
...
2. `catalogRead.server.ts` adds its latest revision, enabled canonical
   ...
   to the private app read model but is disabled for blendCalcAPI v1.
3. `blendCalcAPICatalog.server.ts` rebuilds and sanitizes only the versioned API contract. It
...
| `links.self`              | API serializer                                                             | Stable blendCalcAPI v1 product URL using the canonical GTIN-14.                                                                                                                                                                                                                                                                                                                                                                                                   |
...
Ordinary catalog and blendCalcAPI v1 reads continue to receive only the bounded public-safe
...
`get_blendcalc_api_product_revision_history_v1` is also server-service-role-only. The HTTP
...
blendCalcAPI v1 is not a proxy for USDA, Open Food Facts, or a national dataset. It adds:
