...
an owner-scoped durable job, and schedules recognition through the hosted request's
background-work lifetime using only the job ID. The browser polls a bounded status
response and remains usable throughout; cancellation is an authenticated owner action.
The worker reads DB-backed mappings, persists only structured suggestions, and deletes
the temporary crop. Raw recognized text, OCR image bytes, and moderation evidence never
enter the background handoff or job result. One Tesseract worker is used at a time
within each function runtime, reused briefly for a nearby scan, and terminated after a
short idle period to release memory.
...
| `product_api_cache`                  | Stores keyed USDA and Open Food Facts payloads, status, expiry, and optional ETag; concurrent identical requests are coalesced in-process and by short shared request leases across app instances                                                  | Durable private licensed cache; never exposed by blendCalcAPI and never treated as canonical evidence |
...
| Open Food Facts request policy       | One stable-v2 provider-normalized exact read; field-specific response; 12 requests/minute shared ceiling; no inline retry; exact product: 7 days; product-not-found: 12 hours; stale-on-provider-error/budget exhaustion: up to 30 additional days | Persistent cache plus service-role-only shared lease and budget rows                                  |
...
`product_api_request_leases` prevents two app instances from sending the same expired
or missing cache request at once. `external_provider_request_budgets` atomically keeps
Open Food Facts at 12 reads per minute across instances, below the provider's documented
15-read ceiling. These service-role-only coordination rows contain provider/cache
identities and timing only. If coordination is unavailable, the Open Food Facts adapter
fails closed to stale cache, another provider, or manual completion instead of sending
an unbudgeted request. This policy does not change USDA or COLA Cloud behavior.
