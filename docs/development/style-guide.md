...
| Privileged desktop side gutter | `$app-privileged-desktop-gutter`   | Responsive to `15rem`   |
| Privileged queue-card width    | `$app-privileged-card-grid-*`      | `32rem`–`40rem`         |
...
Privileged-tool right sheets are the deliberate exception to the app shell's narrow
content cap. At widths above `$app-breakpoint-md`, the landing page and every focused
tool use the full viewport canvas inside one shared responsive side gutter that grows
to `15rem`. Focused tools retain a persistent instruction rail and a flexible work
column instead of adding an independent content-width cap. Repeated task and destination
cards stop growing at `40rem`; once another `32rem` card fits, the collection adds a
column rather than stretching buttons, borders, and short copy across empty space.
Cards in the same grid row stretch to the tallest card in that row so their surfaces
and action areas align.
Empty queue states and decision-only forms use the same `40rem` readable bound rather
than filling an otherwise empty work column. Repeated repair and catalog-review records
follow the same grid rule as queue cards. Repeated diagnostic disclosures also wrap at
those bounds, but align to the start while open so expanding one disclosure does not
manufacture empty height in its neighbors. Single comparison or correction workflows may
use more width only when their internal evidence uses that width to communicate a
meaningful relationship. At or below that breakpoint they keep the established
single-column mobile flow. Resizing reflows the same route in place without a mode
prompt, reload, or loss of entered form state; ordinary app right sheets retain their
existing width.

The privileged-tools route family owns one desktop-only utility toolbar in the
persistent left instruction rail. Its Product lookup action opens a bordered, movable
inspector without a backdrop or modal
focus trap, allowing the task underneath to remain readable. The inspector uses the
shared segmented control, actions, status messages, and checkbox primitives; it relies
on surfaces and borders rather than shadow for depth. Stored and live-provider scopes
remain visibly distinct, and comparison rows pair color with explicit difference,
missing, or incompatible-basis text. The inspector is absent at and below the shared
`680px` breakpoint rather than becoming another mobile overlay.
...
| Fixed-choice dropdown                 | `SelectField`                      | Accessible combobox and locally overlaid listbox with shared label, helper, keyboard/typeahead, focus, disabled, and responsive states; a hidden native select preserves required validation and form submission |
...
| Privileged workspace start            | `PrivilegedWorkspaceGuide`         | Current waiting/clear/on-demand state, first step, and explicit completion condition before privileged records                                                                                                   |
| Privileged tool help                  | `PrivilegedToolInformationSheet`   | Contextual usage trigger, review flow, completion condition, action effect, and guardrail opened from one circular info action                                                                                   |
| Catalog review product inbox          | `CatalogReviewProductInbox`        | One compact destination per canonical product/UPC with a total badge and written category breakdown; individual findings stay in the product detail                                                              |
| Desktop product evidence lookup       | `PrivilegedProductLookup`          | Route-wide movable privileged inspector for separately attributed stored/provider search and evidence-safe two-product comparison; hidden at and below `680px`                                                   |
| Catalog conflict decision workbench   | `CatalogConflictDecisionWorkbench` | Desktop-first one-row-per-field evidence comparison with one of four explicit outcomes, automatic nutrient basis conversions, decision progress, and one sticky finish action                                    |
| Catalog review product rail           | `CatalogReviewProductRail`         | Persistent product identity, availability, exact API withholding reasons, pending-correction handoff, and one grouped Data Operations diagnostic link                                                            |
...
- Keep form state intact when the browser loses focus or the user returns from another
  window.
- Start preparing a bounded display preview immediately after a product photo is chosen
  in a dedicated browser worker, without mounting the full-resolution source or
  blocking any form control. Keep the source selection intact and show a clear
  nonblocking status until the complete preview frame is ready. Before catalog
  submission, prepare role-specific upload copies sequentially off the main interface;
  preserve the most detail for nutrition text, bound the combined request below the
  deployment limit, and retain independent server validation. Each photo input moves
  honestly through `Preparing photo`, `Ready`, `Uploading`, `Uploaded`, or
  `Needs attention`; show measured byte progress when the transport provides it and an
  indeterminate progress bar otherwise. Automatic
  image placement is opt-in, starts only after the user selects `Place automatically`,
  and remains cancelable background assistance. While it runs, progress and Stop stay
  visible, and no form, scrolling, evidence upload, navigation, or save control waits
...
- The privileged-tools launcher is a standard Profile link to a full landing page. Keep
  cross-workspace diagnostics and catalog/API reach on that landing page; focused right
  sheets contain the work itself, not a repeated health dashboard.
