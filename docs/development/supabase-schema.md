...
| [Shared Product Catalog](#shared-product-catalog-and-barcode-flow)                            | submissions, products, revisions, observations, provenance, resolution policy, source-field coverage, images, caches, and submission blocks |
...
| `food_nutrient_measurements`               | `id`          | Shared or user-owned depending on parent | Exact nutrient values on a mass, volume, or source-serving basis without inventing a gram conversion                        | Exactly one parent id; `nutrient_id → nutrient_definitions.nutrient_id`         |
...
Stores the authoritative nutrient amount and the exact basis reported by the label or
source. This table prevents volume-only and count-based labels from being converted to
grams without measured evidence.

| Table                        | Documented columns                                                                                                                                                                                                                                                                                                                |
| ---------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `food_nutrient_measurements` | `id`, food-parent and ownership ids, `nutrient_id`, `amount`, `unit_name`, `basis_kind`, `basis_quantity`, `basis_unit_key`, optional `basis_serving_label`, `value_origin`, `value_status`, `value_qualifier`, `source`, `source_reference`, source nutrient identity, mapping and derivation metadata, `confidence`, timestamps |
...
- A source serving such as `1 cookie`, `2 crackers`, or `1 bottle` uses the `serving`
  basis and references the matching `food_servings` row. The readable source label is
  preserved rather than replaced by a guessed mass.
- `sync_native_food_measurements_from_parent` rebuilds exact rows from a parent food
  snapshot. It creates a secondary `food_nutrients` projection only from exact mass
  evidence and marks the row `derived` with `exact-native-basis-to-100g`.
  `skip_non_mass_food_nutrient_projection` prevents native-only nutrient values from
...
- `serving_measure_units` has one enabled default per dimension and indexed display
  ordering.
- Count uses the generic `item` unit while the source label preserves the actual food
  form, such as `1 cookie`, `2 crackers`, `3 dumplings`, or `1 package`.
...
- The nutrition view loads these rows and defaults to the primary source serving. It
  offers a 100g view only when a mass basis or exact measured conversion exists. Mix may
  convert within weight, within volume, or between exact source servings; it crosses
...
| `catalog_conflict_review_dispositions`     | `id`                    | Private catalog review          | Terminal insufficient-evidence outcome for one exact conflict evidence fingerprint                                         | `conflict_id → shared_product_conflicts.id`; `shared_product_id → shared_products.id`  |
...
| `catalog_product_purge_audit`              | `id`                    | Private operations audit        | Non-identifying completion record for an explicitly confirmed destructive product purge                                    | Optional `requested_by → auth.users.id`; intentionally stores no barcode or product id |
...
An MFA-verified data operator can preview and then purge one exact UPC / GTIN through
`preview_catalog_product_purge` and `purge_catalog_product_by_barcode`. The apply
function takes an advisory lock, selects the explicit catalog relationship graph,
removes catalog, evidence, moderation, provider-cache, Storage evidence, and user-owned
copies, and fails closed if primary barcode rows remain. Immutable submission and
provider-snapshot triggers remain enabled; they recognize only the transaction-local
maintenance context established inside the protected purge function. The audit retains
the actor, reason, source, counts, and completion time but deliberately does not retain
the deleted barcode, product id, product name, or source payload.
...
- `queue_resolution` distinguishes deterministic legacy-queue outcomes.
  `already_available` is an accepted no-op because the active catalog already contains
  the submitted values. `stale_revision` and `catalog_now_exists` close an obsolete
  proposal without penalty and require any remaining change to be resubmitted against
...
| `privileged_queue_admission_decisions` | `id`, `queue_name`, `subject_type`, `subject_key`, `evidence_fingerprint`, `outcome`, `reason_code`, `evidence_snapshot`, `created_at`                                                                                              |
...
- `privileged_queue_admission_decisions` is immutable, forced-RLS, and service-only. It
  records conservative automatic queue resolutions and single-owner routing without
  replacing the underlying submission, provider snapshot, conflict, or repair audit.
- `apply_catalog_queue_admission` closes only stale provider snapshots, provider values
  already reflected by exact canonical values/evidence, and normalized-equivalent
  product-name or brand conflicts. `resolve_catalog_submission_queue_admission` accepts
  only a current product/revision evidence snapshot produced by the trusted server.
...
`catalog_conflict_review_dispositions` records that an MFA-verified catalog reviewer
could not determine a field from the currently stored evidence. Its fingerprint includes
the conflict identity, product, field, observed values, and severity. The service-only
`catalog_actionable_product_conflicts` view excludes a matching unchanged fingerprint,
conflicts linked to a pending correction, and provider-generated conflicts already
represented by their owning provider-change review. The companion
`catalog_actionable_provider_change_reviews` view excludes reviews already owned by a
correction workflow. Underlying evidence remains intact; changed material evidence
automatically returns to the appropriate single queue owner. Rows are private and do
not grant browser access to source evidence.
...
Accuracy-sensitive product matching, field selection, difference severity, category
suggestion, USDA subtype ranking, and nutrition-completeness scoring share one reviewed
policy version instead of private runtime constants.

| Table                                      | Primary key                          | Purpose                                                                                      |
| ------------------------------------------ | ------------------------------------ | -------------------------------------------------------------------------------------------- |
| `product_resolution_policy_versions`       | `key`                                | Owns the active reviewed version and its identity, numeric, serving, and category boundaries |
| `product_resolution_rank_values`           | Policy, ranking context, value       | Stores confidence and provider-subtype ordering                                              |
| `product_resolution_scoring_weights`       | Policy, scoring context, metric      | Stores field-completeness and category-candidate weights                                     |
| `product_resolution_difference_thresholds` | Policy, comparison context, severity | Stores ordered low, medium, and high comparison boundaries                                   |
| `product_resolution_ignored_terms`         | Policy, term context, term           | Stores reviewed noise terms used by bounded matching                                         |
| `product_source_field_coverage_policies`   | Policy and provider                  | Sets separate expiry periods for reported fields, absent fields, and exact product misses    |
| `product_source_field_coverage`            | Barcode, provider, field             | Records an expiring successful lookup outcome for one exact product field                    |

`nutrition_completeness_profiles` links each profile to the reviewed assessment policy
and owns exact, mapped, derived, and missing scores; required and recommended weights;
and the partial-label boundary. Their database constraints preserve score ordering.

Source-field coverage is server-only operational state. It may prevent a repeated
provider request while its expiry is fresh, but it is not provenance, verification,
canonical evidence, or proof that a field can never become available. Errors never
create absence coverage. The composite policy/provider relationship prevents a lookup
row from using expiry rules that were not reviewed for that provider.
...
- Incorrect-warning intake records the exact current shared-product revision when one
  ...
  confirmed fact snapshot exactly matches canonical `product_compatibility_facts`.
  Other report reasons, missing or changed revisions, changed policies, absent facts,
  non-confirmed facts, and mismatched evidence remain pending.
- `decision_method` distinguishes `human` decisions from
  `system_exact_evidence`; `decision_metadata` preserves the rule version, stable
  decision code, policy version, product revision, and matched canonical fact IDs.
  Completed decision fields are immutable. System decisions have no human reviewer and
  always dismiss with no follow-up.
...
| `privileged_queue_admission_decisions`   | `id`                   | Preserves immutable deterministic queue-resolution and routing receipts                                 |
...
- A bounded service admission pass supersedes an older pending provider review, or one
  whose complete changed-field set is already reflected by the current canonical
  values or selected exact observation. Provider-derived conflict rows are owned by
  that provider review rather than counted as additional decisions. Unsupported or
  ambiguous cross-shape comparisons remain pending.
...
`FDA_RECALL_PROXY_URL` and `FDA_RECALL_PROXY_SECRET` retain their original compatibility
...
- `get_privileged_tool_action_summary()` returns exact AAL2, live-role-aware Profile
  ...
  Disabled issue codes, accepted publication-only diagnostics, and nonblocking missing
  historical revision-audit details are excluded from required-work counts. The latter
  remain queryable through catalog-health diagnostics and product passports. It
  excludes search-only Account access and sums only the counts permitted for the current
  database role assignment.
- Catalog-review counts use the same actionable conflict and provider-review views as
  the workspace itself, so filtered, routed, or correction-owned rows cannot leave a
  phantom red badge.
...
| `generic_food_dataset_import_evidence_runs` | `id`                                                | Preserves AAL2 dataset evidence previews and applies                   | Append-only mode/outcome, exact release snapshot, before/proposed evidence, HTTPS reference, actor, private apply note; canonical evidence remains on `generic_food_datasets`     |
...
| `get_dataset_import_evidence_workspace`                | Returns one bounded dataset release, current import/checksum evidence, missing artifacts, and latest apply receipt after exact AAL2 data-repair authorization                                                                   |
| `preview_dataset_import_evidence`                      | Validates one release-bound UTC import time, SHA-256, and HTTPS evidence reference and records an append-only preview without changing canonical dataset state                                                                  |
| `apply_dataset_import_evidence`                        | Applies one current same-operator preview, fills only missing canonical dataset evidence, records the private audit decision, and rolls back unless the owning health finding clears                                            |
...
| `get_catalog_product_api_withholding_reasons`          | Returns the exact current public-API withholding reasons for one product after AAL2 catalog-review or Data Operations authorization                                                                                             |
| `finish_catalog_conflict_review`                       | Atomically records one evidence-backed outcome for every actionable conflict and creates at most one pending correction submission containing all selected replacements                                                         |
...
| `get_privileged_tool_action_summary`                   | Returns exact live-role-aware actionable Profile counts after AAL2 verification, deduplicating data-operations work by affected subject and excluding search-only access plus nonblocking historical revision-audit diagnostics |
