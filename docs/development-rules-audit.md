...
42. <a id="rule-icon-components"></a>All app UI icons must be reusable Svelte components under `src/lib/assets/icons` or a clearly named nested icon folder. Avoid raw emoji, one-off inline SVG, and ad hoc glyph spans in rendered UI unless the symbol is user-facing text content rather than an icon.
43. <a id="rule-qa-links"></a>QA tasks must include exact reproduction steps, exact code references, and links to the relevant development rules. Do not create duplicate rule sets inside QA docs; reference this document as the source of truth.
