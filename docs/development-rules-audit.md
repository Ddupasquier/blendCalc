...
42. <a id="rule-qa-clearance"></a>Finished tasks must prompt the user to run the relevant QA checks from local `docs/QA/qa-tasks.md`. Keep QA sections active until the user explicitly confirms every item in that section passed. When a whole section is complete, move the full section to `docs/QA/completed-qa-tasks.md` and remove it from active QA. Do not silently delete active QA coverage and do not move partial sections.
