...
8a. <a id="rule-circular-icon-alignment"></a>Every icon inside a circular container must be centered both horizontally and vertically by a shared primitive. Interactive circles must use `CircleIconButton`, `CloseButton`, or another approved shared button; non-interactive status circles must use a shared badge/media primitive such as `StatusIconBadge`. Circular primitives must use equal width and height, `inline-grid` or `grid` with `place-items: center`, token-backed container and icon sizing, and `line-height: 1`. Do not center icons with glyph whitespace, manual margins, transforms, or browser-default button alignment, and do not recreate circular icon wrappers inside feature components.
...
9a. <a id="rule-destructive-action-confirmation"></a>Destructive actions that immediately remove user data must use a shared two-step confirmation flow. The first activation must not change data; it must arm the action, visibly explain that the user must tap or click delete again, and update the control's accessible label. Only the second activation may run the delete. Apply the same guard to every route into the action, including compact card controls and ingredient action sheets. Use `TwoStepConfirmation.svelte` rather than feature-local timers, double-click event handlers, or one-off confirmation messages. The armed state must expire automatically, preserve keyboard access, show a busy/disabled state during the real delete, and use shared design tokens.
