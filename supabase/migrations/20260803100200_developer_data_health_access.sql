...
begin
	if auth.uid() is null or not exists (
		...
		where role_assignment.user_id = auth.uid()
			and role_assignment.role in ('moderator', 'admin', 'developer')
		) then
...
comment on function public.get_moderator_data_health(integer, integer) is
	'Moderator, admin, and developer bounded data-health aggregates and issue summaries. Raw provider payloads, private evidence, user identities, and secrets are excluded.';
