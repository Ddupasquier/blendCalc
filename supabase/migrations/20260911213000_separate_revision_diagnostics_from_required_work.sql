...
comment on function public.get_privileged_tool_action_summary() is
	'Returns exact role-filtered privileged action counts and a bounded named data-operations subject list after AAL2, including every open food-warning follow-up that remains in the focused queue; historical revision-audit diagnostics remain outside the required-work count.';
