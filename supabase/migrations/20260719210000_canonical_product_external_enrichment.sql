alter table public.product_data_sources
	add column if not exists canonical_storage_allowed boolean not null default false,
	add column if not exists canonical_license_name text,
	add column if not exists canonical_policy_reviewed_at timestamptz,
	add column if not exists canonical_policy_notes text;

update public.product_data_sources
set canonical_storage_allowed = true,
	canonical_license_name = 'CC0-1.0',
	canonical_policy_reviewed_at = now(),
	canonical_policy_notes = 'USDA FoodData Central data is public domain/CC0 and may be stored in the blendCalc canonical product dataset.'
where key = 'usda';

update public.product_data_sources
set canonical_storage_allowed = false,
	canonical_license_name = null,
	canonical_policy_reviewed_at = now(),
	canonical_policy_notes = 'Keep provider data in its licensed cache or asset path until the downstream public-database licensing model is deliberately accepted.'
where key = 'open-food-facts';
...
begin
	...
	if exists (
		...
		where field_name not in ('nutrition', 'image', 'categories', 'serving')
	) then
	...
	v_canonical_provenance := coalesce(v_product.canonical_provenance, '{}'::jsonb);
	...
	if 'image' = any(coalesce(p_candidate_fields, '{}'::text[]))
		...
		and nullif(btrim(p_enriched_food -> 'image' ->> 'imageUrl'), '') is not null then
	...
	if 'categories' = any(coalesce(p_candidate_fields, '{}'::text[]))
		...
		and exists (
			...
			where category.id = p_category_option_id and category.enabled
		) then
		foreach v_key in array array['foodCategory', 'categories', 'categoryOptionId']
		loop
	...
	if 'serving' = any(coalesce(p_candidate_fields, '{}'::text[]))
		...
		and (
			coalesce(p_enriched_food ->> 'customServingWeightGrams', '') ~ '^[0-9]+([.][0-9]+)?$'
			or coalesce(p_enriched_food ->> 'servingSize', '') ~ '^[0-9]+([.][0-9]+)?$'
		) then
	...
	set food = v_food,
		category_option_id = case
			when 'categories' = any(v_applied_fields) then p_category_option_id
