...
begin
	...
	if 'nutrition' = any(coalesce(p_candidate_fields, '{}'::text[]))
		and (
			jsonb_typeof(v_food -> 'foodNutrients') is distinct from 'array'
			or jsonb_array_length(v_food -> 'foodNutrients') = 0
		)
		and jsonb_typeof(p_enriched_food -> 'foodNutrients') = 'array'
		and jsonb_array_length(p_enriched_food -> 'foodNutrients') > 0 then
		v_food := jsonb_set(
			v_food,
			'{foodNutrients}',
	...
	if 'ingredients' = any(coalesce(p_candidate_fields, '{}'::text[])) then
		...
		if nullif(btrim(v_food ->> 'ingredients'), '') is null
			and nullif(btrim(p_enriched_food ->> 'ingredients'), '') is not null then
			v_food := jsonb_set(
				v_food,
				'{ingredients}',
		...
		if (
			jsonb_typeof(v_food -> 'ingredientList') is distinct from 'array'
			or jsonb_array_length(v_food -> 'ingredientList') = 0
		)
		and jsonb_typeof(p_enriched_food -> 'ingredientList') = 'array'
		and jsonb_array_length(p_enriched_food -> 'ingredientList') > 0 then
			v_food := jsonb_set(
				v_food,
				'{ingredientList}',
	...
	loop
		if v_field = any(coalesce(p_candidate_fields, '{}'::text[]))
			and (
				...
				or jsonb_array_length(v_food -> v_field) = 0
			)
			and jsonb_typeof(p_enriched_food -> v_field) = 'array'
			and jsonb_array_length(p_enriched_food -> v_field) > 0 then
			v_food := jsonb_set(
				v_food,
				array[v_field],
	...
	loop
		...
		if v_field = any(coalesce(p_candidate_fields, '{}'::text[]))
			and (
				...
				or v_food -> v_key = '{}'::jsonb
			)
			and jsonb_typeof(p_enriched_food -> v_key) = 'object'
			and p_enriched_food -> v_key <> '{}'::jsonb then
			v_food := jsonb_set(
				v_food,
				array[v_key],
				p_enriched_food -> v_key,
