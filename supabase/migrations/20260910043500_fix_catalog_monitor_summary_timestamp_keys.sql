...
begin
	return jsonb_build_object(
		...
		'providerChanges', coalesce((
			select jsonb_agg(to_jsonb(change_row) order by change_row."createdAt")
			from (
				select
					review.id,
		...
		'safetyMatches', coalesce((
			select jsonb_agg(to_jsonb(match_row) order by match_row."detectedAt")
			from (
				select
					match.id,
...
comment on function private.build_catalog_monitor_summary(integer) is
	'Builds the bounded catalog-monitor summary with camelCase application fields and without deciding caller authorization. Public wrappers must enforce their own exact permission.';
